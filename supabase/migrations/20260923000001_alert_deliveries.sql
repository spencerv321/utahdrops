...
create index if not exists alert_deliveries_event on alert_deliveries (event_id);

-- server-only, like scrape_runs
alter table alert_deliveries enable row level security;

-- the digest filters events by type + recency
create index if not exists inventory_events_type_created on inventory_events (event_type, created_at desc);
