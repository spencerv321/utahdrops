...
/** "Just happened": recent restocks, new bottles, price drops and clearance. */
  ...
  if (events.length === 0) {
    return (
      <p className="rounded-2xl border border-dashed p-8 text-center text-muted-foreground">
        Quiet week. Restocks, new bottles and price drops show up here as DABS updates.
      </p>
    );
  }
  return (
    <ul className="grid gap-2.5 lg:grid-cols-2">
      {events.map((e) => {
        const name = displayName(e.name ?? e.csc ?? "");
        return (
          <li key={e.id} className="flex gap-1 rounded-2xl border bg-card py-3 pr-1.5 pl-4">
            <Link href={`/product/${e.csc}`} className="min-w-0 flex-1 space-y-1.5">
